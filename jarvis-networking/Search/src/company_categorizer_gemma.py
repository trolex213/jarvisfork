"""
Company Categorization System using Gemma LLM and Google Search API

This script categorizes companies into standardized categories using a hybrid approach:
1. First checks against known patterns (like NYU Stern → Education)
2. Falls back to Gemma 3 LLM for classification
3. Optionally supplements with Google Search results

Usage Examples:
--------------
1. Basic CSV processing:
   python company_categorizer_gemma.py --input companies.csv

2. Clean reprocessing (delete existing outputs):
   python company_categorizer_gemma.py --input companies.csv --clean

3. Append to existing results:
   python company_categorizer_gemma.py --input new_companies.csv --append

4. Custom output paths:
   python company_categorizer_gemma.py --input companies.csv \
          --output custom_categories.json \
          --raw-output raw_responses.json

5. Large batch processing:
   python company_categorizer_gemma.py --input large_list.csv --batch-size 50
"""
...
class CompanyCategorizerGemma:
    """
    Main company categorization engine using Gemma LLM.
    
    Features:
    - Batch processing of company names
    - Progress tracking and resumption
    - Google Search API integration
    - Local caching of results
    
    Args:
        output_file (str): Path for categorized results (JSON)
        raw_file (str): Path for raw LLM responses (JSON)
        batch_size (int): Number of companies processed per request (default: 30)
    
    Methods:
        categorize_companies(companies): Main processing method for company lists
        make_batch_request(batch): Process a single batch through LLM
        web_search(query): Perform Google search with caching
    
    Example:
        >>> categorizer = CompanyCategorizerGemma()
        >>> results = categorizer.categorize_companies(["Google", "NYU Stern"])
    """
    ...
    def __init__(self, output_file='company_categories_gemma.json', 
                 raw_file='company_categories_raw.json', batch_size=30):
        """
        Initialize categorizer with output paths and processing settings.
        
        Args:
            output_file: JSON output path for categorized results
            raw_file: JSON output path for raw LLM responses
            batch_size: Number of companies to process per API call
        """
    ...
    def _verify_category(self, category):
        ...
        valid_categories = [c.lower() for c in self.VALID_CATEGORIES]
...
def load_existing_results(output_file: str) -> Dict:
    ...
    if os.path.exists(output_file):
        try:
            ...
            print(f"Warning: Could not load {output_file} - {str(e)}")
    return {}
...
def parse_args():
    ...
    parser.add_argument('--raw-output', default='company_categories_raw.json',
                      help='Output JSON file for raw responses')
    parser.add_argument('--clean', action='store_true',
                      help='Delete all output files before processing')
    parser.add_argument('--append', action='store_true',
                      help='Append to existing output file')
    parser.add_argument('--batch-size', type=int, default=10,
                      help='Companies to process per batch')
...
def main():
    ...
    # Handle clean flag
    if args.clean:
        for f in [PROGRESS_FILE, args.output, args.raw_output]:
            try:
                if os.path.exists(f):
                    os.remove(f)
                    print(f"Cleaned: {f}")
    ...
    existing_results = load_existing_results(args.output) if args.append else {}
    ...
    if args.append:
        all_companies = [c for c in all_companies if c not in existing_results]
    
    # Process and merge results
    new_results, raw_responses = categorizer.categorize_companies(all_companies)
    ...
    print(f"Processed {len(new_results)} new companies")
    print(f"Total companies in {args.output}: {len(final_results)}")
