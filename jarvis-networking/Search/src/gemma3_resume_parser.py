...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description='Process a single resume PDF and generate connection points')
    parser.add_argument('pdf_path', help='Path to the resume PDF file')
    args = parser.parse_args()

    resume_data = parse_resume_with_retry(args.pdf_path)
    ...
    if resume_data:
        ...
        if connections:
            ...
            connections_path = save_to_json(
                ...
                args.pdf_path.replace(".pdf", "_connections.json")
            )
